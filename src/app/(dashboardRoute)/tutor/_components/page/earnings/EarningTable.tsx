...
interface Props {
  ...
  activeFilter: TutorPaymentStatus | "ALL";
  onResetFilter: () => void;
}

const STATUS_BADGE: Record<TutorPaymentStatus, { label: string; className: string }> = {
...
export function EarningTable({ payments, loading, sorting, onSortingChange }: Props) {
  const columns: ColumnDef<TutorPayment>[] = [
    {
      accessorKey: "student",
      header: "Student",
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: "Payment",
      cell: ({ getValue }) => {
        const { label, className } = STATUS_BADGE[getValue<TutorPaymentStatus>()] ?? { label: "Unknown", className: "" };
